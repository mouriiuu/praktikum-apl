...
void doLogin(int &loginBerhasil) {
    ...
    try {
        throw runtime_error("Login gagal. Program berhenti.");
...
void doRegister(user userArr[], int &jumlah) {
    ...
    try {
        ...
        if (jumlah >= MAX_USER)
            throw length_error("Kapasitas user sudah penuh!");
...
void lihat(perjalanan perjalananArr[], int jumlah) {
    ...
    if (pilihSearch == 1) {
        ...
        try {
            long long target = validasiBudget(input);
        ...
        try {
            if (isAngka(input))
                throw invalid_argument("Masukkan huruf!");
        } catch (const invalid_argument& e) {
            cout << e.what() << endl;
            return;
        }
...
void tambah(string *namaUser) {
    ...
    try {
        if (jumlahPerjalanan >= MAX_PERJALANAN)
            throw length_error("Kapasitas perjalanan sudah penuh!");
...
void edit(string namaUser) {
    ...
    while (no < 1 || no > count) {
        ...
        if (input.empty()) { cout << "Kembali ke menu." << endl; return; }
        try {
            if (!isAngka(input)) throw invalid_argument("Pilihan tidak valid!");
            no = stoi(input);
            if (no < 1 || no > count) throw out_of_range("Pilihan tidak valid!");
        } catch (const invalid_argument& e) {
            cout << e.what() << endl;
            no = 0;
        } catch (const out_of_range& e) {
    ...
    while (true) {
        ...
        try {
            daftarPerjalanan[i].budget = validasiBudget(input);
            break;
        } catch (const invalid_argument& e) {
            cout << e.what() << endl;
        } catch (const out_of_range& e) {
...
void menuUtama(string &namaUser) {
    ...
    do {
        ...
        try {
            if (input.empty()) throw invalid_argument("Tidak boleh kosong!");
            if (!isAngka(input)) throw invalid_argument("Pilihan tidak valid!");
            pilih = stoi(input);
            if (pilih < 1 || pilih > 5) throw out_of_range("Pilihan tidak valid!");
        } catch (const invalid_argument& e) {
            cout << e.what() << endl;
            pilih = 0; continue;
        } catch (const out_of_range& e) {
...
int main() {
    ...
    do {
        ...
        try {
            ...
            if (!isAngka(input)) throw invalid_argument("Pilihan tidak valid!");
            pilih = stoi(input);
            if (pilih < 1 || pilih > 3) throw out_of_range("Pilihan tidak valid!");
        } catch (const invalid_argument& e) {
            cout << e.what() << endl;
            pilih = 0; continue;
        } catch (const out_of_range& e) {
